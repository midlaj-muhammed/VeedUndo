        ...
        <div className="w-full max-w-md bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] p-8">
          {sent ? (
            <div className="text-center">
              ...
              <button onClick={() => { setSent(false); setEmail(""); }} className="text-sm text-[var(--color-primary)] hover:underline">Use a different email</button>
            </div>
          ) : (
            <>
              ...
              {error && <div className="mb-4 rounded-lg bg-red-500/10 border border-red-500/20 px-4 py-3 text-sm text-red-600 dark:text-red-400">{error}</div>}
