...
export default function ListingDetail() {
  ...
  async function handleFlag() {
    ...
    if (!session) {
      alert("Please sign in to flag a listing.");
      setFlagging(false);
      return;
    }
    ...
    <div className="flex flex-col min-h-dvh">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-8">
        ...
        {listing.image_urls?.length > 0 && (
          <div className="mb-6">
            <img
              src={listing.image_urls[selectedImg]}
              alt={imgAlt}
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          <span className="rounded-full bg-[var(--color-muted)] px-3 py-1 text-sm font-medium text-[var(--color-text)]">
        ...
        {/* Description */}
        {listing.description && (
          <p className="text-[var(--color-text)] mb-6 whitespace-pre-wrap leading-relaxed">
            {listing.description}
          </p>
        )}

        {/* Contact */}
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 mb-6 shadow-sm">
          <h2 className="font-bold mb-4 text-lg">Contact</h2>

          {listing.poster_whatsapp && (
            <a
              href={`https://wa.me/91${listing.poster_whatsapp.replace(/^0/, "")}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full rounded-xl bg-[#25D366] px-5 py-3 text-sm font-medium text-white hover:bg-[#1da851] transition-colors mb-3 cursor-pointer"
            >
          ...
          {listing.poster_phone && (
            <a
              ...
              className="flex items-center justify-center gap-2 w-full rounded-xl border border-[var(--color-border)] px-5 py-3 text-sm font-medium text-[var(--color-text)] hover:border-[var(--color-primary-light)] hover:text-[var(--color-primary)] transition-colors mb-2 cursor-pointer"
            >
        ...
        <div className="text-center">
          {flagged ? (
            <p className="text-sm text-[var(--color-text-muted)]">
              Thanks for reporting this listing.
            </p>
          ) : (
            <button
              onClick={handleFlag}
              disabled={flagging}
              className="text-sm text-[var(--color-text-muted)] hover:text-[var(--color-destructive)] underline disabled:opacity-50 cursor-pointer"
            >
