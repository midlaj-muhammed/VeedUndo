...
export default function Home() {
  ...
  useEffect(() => {
    supabase.from("localities").select("*").order("name").then(({ data }) => {
  ...
  useEffect(() => {
    ...
    query.then(({ data, error: err }) => {
      ...
      else { setListings((data as ListingWithLocality[]) || []); }
      setLoading(false);
    });
  }, [filters]);
      ...
      <header className="hero-gradient border-b border-[var(--color-border)] bg-[var(--color-surface)]">
        <div className="max-w-6xl mx-auto px-4 py-12 sm:py-16 text-center relative">
          ...
          <div className="flex items-center justify-center gap-4">
            <Link href="/post" className="press-effect rounded-full bg-[var(--color-primary)] px-6 py-3 text-sm font-medium text-white hover:bg-[var(--color-primary-dark)] shadow-lg shadow-[var(--color-primary)]/20">
              + Post a Listing
            </Link>
            <a href="#listings" className="press-effect rounded-full border border-[var(--color-border)] px-6 py-3 text-sm font-medium text-[var(--color-text)] hover:border-[var(--color-primary-light)] hover:text-[var(--color-primary)]">
              Browse Listings
            </a>
          </div>
        </div>
      ...
      <main id="listings" className="flex-1 max-w-6xl mx-auto w-full px-4 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => window.location.reload()} className="text-sm text-[var(--color-primary)] hover:underline">Try again</button>
          </div>
        ) : listings.length === 0 ? (
          <div className="text-center py-16 bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)]">
            <svg className="w-16 h-16 mx-auto mb-4 text-[var(--color-text-dim)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <Link href="/post" className="press-effect inline-block rounded-full bg-[var(--color-primary)] px-5 py-2.5 text-sm font-medium text-white hover:bg-[var(--color-primary-dark)]">Post a Listing</Link>
          </div>
        ) : (
          <>
